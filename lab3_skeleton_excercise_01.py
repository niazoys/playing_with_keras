...
def plot_curves(history,class_1,act):
    ...
    plt.savefig(
        "training-validation-Loss--single_neuron-binary-classification-{0}-VS- all -{1}-epoches_"+str(act)+".png".format(str(class_1),str(epochs)))
    ...
    plt.savefig(
        "training-validation-Accuracy--single neuron-binary-classification-{0}-VS- all -{1}-epoches.png".format(str(class_1),str(epochs)))

def buildNetwork(input_dim):
    model = Sequential()
    # Adding the input layer and the first hidden layer with dropout
    model.add(Dense(units = 1, activation = 'sigmoid', input_dim =784))
...
if __name__ == "__main__":
    ...
    filepath= "./accuracy_excercise_01.txt"
    Accuracy_list=[]
    # hyperparameters
    ...
    for batch in batch_list:
        #Compile the model 
        model=buildNetwork(in_dim)

        # train model
        history = train_model(model,X_train[1:10,:],Y_train[1:10,],validation_split_size,learning_rate,batch,epochs)

        # plot training loss& accuracy
        plot_curves(history,class_1,batch)

        #Find the accuracy on test data
        _, accuracy = model.evaluate(X_test[1:10,:], Y_test[1:10,])
        
        print("=== Accuracy with batch size"+str(batch)+"= {:.2f}=== %".format(accuracy * 100))
        Accuracy_list.append(accuracy)
        
    with open(filepath, 'w') as f:
        for i,b in enumerate (batch_list):
            f.write("Accuracy with batch size ("+str(b)+"): ")
